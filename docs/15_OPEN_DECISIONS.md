...
The following twenty (20) decisions are formally **OPEN**. For each item, the implementation mandate is: **"Implementation must not invent this policy."**
...
| `OD-019` | **Unregistered SSO Identity Ingestion Policy** | **PROVISIONAL PROTOTYPE DEFAULT**: Reject login with HTTP 403 (`ERR_AUTH_UNREGISTERED_IDENTITY`) until provisioned in `users` by Administrator or official ERP roster import. | Determines whether newly authenticated SSO users are auto-provisioned with blank roles or rejected until verified. | `docs/16_IDENTITY_AUTHENTICATION_ARCHITECTURE.md`<br>`docs/17_RBAC_RLS_BEHAVIORAL_TEST_MATRIX.md` | NIET Academic Governance / ERP Team | *Implementation must not invent this policy.* Enforce pre-provisioning requirement in V1 to prevent unverified student/faculty accounts. | `OPEN` |
| `OD-020` | **Multi-Role Session Context Switching** | **PROVISIONAL PROTOTYPE DEFAULT**: Composite multi-role evaluation at database RLS layer (`public.has_role()`) with optional active role switcher in user navigation. | Governs whether faculty holding multiple roles (e.g. DC + Guide) must manually switch roles or enjoy merged permissions. | `docs/16_IDENTITY_AUTHENTICATION_ARCHITECTURE.md`<br>`docs/04_RBAC_MATRIX.md` | NIET HOD Committee / UX Team | *Implementation must not invent this policy.* Evaluate all active roles union-wise in RLS; support UI active-role header filtering. | `OPEN` |
